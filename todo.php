...
<html lang="en">
    <head>
        ...
        <style>
            ...
            .oi-trash { cursor: pointer; }
            .oi-trash:hover { color: orangered; }
    ...
    <body>
        ...
        <script>
            ...
            /* TO-DO LIST VIEWING AND CREATION */
            // Display lists
            $(document).on('click', '#todo-overview .list-group-item', function() {
                if ($(this).hasClass('add-stuff') && $('#todo-overview .new-stuff').length < 1) {
                    // Create a new to-do list

                    var add_stuff = '<li class="list-group-item new-stuff">\
                        <div class="input-group">\
                            <span class="input-group-addon"><span class="oi oi-check" title="Create to-do list" aria-hidden="true"></span></span>\
                            <input type="text" class="form-control" autofocus="autofocus">\
                        </div>\
                    </div>';
                    $('#todo-overview .add-stuff').before(add_stuff);
            ...
            $(document).on('click', '#todo-overview .input-group-addon', function() {
                var list_name = $(this).siblings('.form-control').val();

                if (list_name.length > 0) {

                    // Add to DB
                    $.post("assets/ajax/todo_create.php", { list_name: list_name }).done(function(data) {
                        $('#todo-overview .add-stuff').before(data);
                    });

                    // Remove input
                    $('#todo-overview .new-stuff').remove();

                }
            });
            
            // Remove to-do list
            $(document).on('click', '.oi-trash', function() {
                var list_id = $('#list-header').attr('list-id'),
                    list_name = $('#list-header h2').text();
                
                // Ask for confirmation
                if (confirm("Are you sure you want to delete the '"+list_name+"' to-do list?")) {
                    $.post("assets/ajax/todo_delete.php", { list_id: list_id }).done(function() {
                       location.reload();  
                    });
                }
            });
            
            /* TO-DO ITEM STATUS CHANGE AND ADDITION */
